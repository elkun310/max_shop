...
@section('product','active')
	<!-- main -->
	<!--search-->
	{{-- <div class="search-box" style="margin-bottom: 20px;float: right;margin-right: 105px;">
		<form action="/search" method="get">
			<input id="txb" name="keyword" type="text" class="sb-text" placeholder="Nhập sản phẩm cần tìm kiếm ...">
			<button id="btn-search" type="submit" class="sb-submit" ><i class="fa fa-search" style="font-size: 20px;"></i></button>
		</form>
	</div> --}}
	<div class="search-box" style="margin-bottom: 40px;float: right;margin-right: 105px;">
		<form action="/search" method="get" id="form-seacrh">
			<input id="txb" name="keyword" type="text" class="sb-text open" placeholder="Nhập sản phẩm cần tìm kiếm ...">
		...
		<div class="container">
			<div class="row">
				<div class="col-md-9 col-md-push-3">
                    <h4 style="font-weight:bold">Tìm kiếm với từ khóa : <span style="color:red">{{$keyword}}</span></h4>
								...
								<div class="size-wrap">
									<p class="size-desc">
										@foreach ($attr->values as $value)
											<a href="/search?value={{$value->id}}" class="attr">{{$value->value}}</a>
										@endforeach
									</p>
								</div>
                            </div>
